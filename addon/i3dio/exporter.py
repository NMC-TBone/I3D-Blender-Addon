...
from .export_core.builder import IRBuilder
from .export_core.ctx import ExportContext
from .export_core.serialize_i3d import write_i3d
from .i3d import I3D
    ...
    try:
        with log_ctx:
            ...
            if source_collection:
                ...
                _export_collection_content(ctx, source_collection)
            else:
                match operator.selection:
                    case "ALL":
                        _export_active_scene_master_collection(ctx)
                    case "ACTIVE_COLLECTION":
                        _export_active_collection(ctx)
                    case "ACTIVE_OBJECT":
                        _export_active_object(ctx)
                    case "SELECTED_OBJECTS":
                        _export_selected_objects(ctx)

            write_i3d(ctx, filepath)
...
def _export_active_scene_master_collection(context: bpy.types.Context, ctx: ExportContext):
    logger.info("'Master Collection' export is selected")
    _export_collection_content(ctx, context.scene.collection)


def _export_active_collection(context: bpy.types.Context, ctx: ExportContext):
    logger.info("'Active collection' export is selected")
    _export_collection_content(ctx, context.view_layer.active_layer_collection.collection)


def _export_collection_content(ctx: ExportContext, collection):
    # First export child collections. Collections are not sorted alphabetically in the blender outliner
    _export(ctx, collection.children.values(), sort_alphabetical=False)
    ...
    _export(ctx, [obj for obj in collection.objects if obj.parent is None])


def _export_active_object(context: bpy.types.Context, ctx: ExportContext):
    logger.info("'Active Object' export is selected")
    if context.active_object is not None:
        _export(ctx, [context.active_object])
    else:
        logger.warning("No active object, aborting export")


def _export_selected_objects(context: bpy.types.Context, ctx: ExportContext):
    if not context.selected_objects:
        logger.warning("No objects selected for export")
        return
    if ctx.settings["selection_traverse_children"]:
        logger.info("'Selected Objects' export is selected, traversing children")
        _export(ctx, context.selected_objects)
    else:
        logger.info("'Selected Objects' export is selected, exporting only selected objects without children traversal")
        _export_selected_only(ctx, context.selected_objects)


def _export_selected_only(ctx: ExportContext, selected_objs: list[BlenderObject]):
    """Export only the selected objects, without traversing their children."""
    selection_set = set(selected_objs)
    ctx.all_objects_to_export.extend(selected_objs)
    ...
    for obj in selected_objs:
        ...
        if parent and parent in selection_set:
            ...
            roots.append(obj)
    ctx._selection_set = selection_set
    ctx._parent_map = parent_map
    for root in sort_blender_objects_by_outliner_ordering(roots):
        _add_object_to_i3d(ctx, root)


def _export(ctx: ExportContext, objects: List[BlenderObject], sort_alphabetical: bool = True):
    ...
    existing_objects = set(ctx.all_objects_to_export)
    ctx.all_objects_to_export.extend([obj for obj in _all_objects if obj not in existing_objects])

    for blender_object in objects_to_export:
        _add_object_to_i3d(ctx, blender_object)


def _add_object_to_i3d(ctx: ExportContext, obj: BlenderObject, parent: SceneGraphNode = None) -> None:
    # Collections are checked first since these are always exported in some form
    if isinstance(obj, bpy.types.Collection):
        logger.debug(f"[{obj.name}] is a 'Collection'")
        node = None
        if ctx.settings["keep_collections_as_transformgroups"]:
            node = ctx.add_transformgroup_node(obj, parent)
        else:
            ctx.logger.info(f"[{obj.name}] will be ignored and its children will be added to nearest parent")
        _process_collection_objects(ctx, obj, node)
    ...
    if obj.i3d_motion_path_array.enabled:
        export_motion_path_array(obj, ctx.depsgraph, logger=ctx.logger)
    # Check if object should be excluded from export (including its children)
    if obj.i3d_attributes.exclude_from_export:
        ctx.logger.info(f"Skipping [{obj.name}] and its children. Excluded from export.")
        return
    ...
    type_supported = obj.type in ctx.settings["object_types_to_export"]
    ...
    match obj.type:
        case "MESH" if type_supported:
            ...
            if "MERGE_CHILDREN" in ctx.settings["features_to_export"] and obj.i3d_merge_children.enabled:
                if obj.children and any(child.type == "MESH" for child in obj.children):
                    logger.debug(f"Processing MergeChildren for: {obj.name}")
                    node = ctx.add_merge_children_node(obj, _parent)
            ...
            elif "MERGE_GROUPS" in ctx.settings["features_to_export"] and obj.i3d_merge_group_index > -1:
                blender_merge_group = bpy.context.scene.i3dio_merge_groups[obj.i3d_merge_group_index]
                ctx.merge_groups.setdefault(
                    obj.i3d_merge_group_index, MergeGroup(xml_i3d.merge_group_prefix + blender_merge_group.name)
                )
                node = ctx.add_merge_group_node(obj, _parent)

            # Process Skinned Meshes if enabled and MergeChildren wasn't applied
            elif (
                "SKINNED_MESHES" in ctx.settings["features_to_export"]
                and "ARMATURE" in ctx.settings["object_types_to_export"]
                ...
                if armature_mod.object is None:
                ...
                elif armature_mod.object not in ctx.all_objects_to_export:
                    ctx.logger.warning(
                    ...
                    node = ctx.add_skinned_mesh_node(obj, _parent)

            # Default to a regular Shape if none of the special types applied
            else:
                node = ctx.add_shape_node(obj, _parent)
        case "ARMATURE" if type_supported:
            node = ctx.add_armature_from_scene(obj, _parent)
        case "EMPTY" if type_supported:
            node = ctx.add_transformgroup_node(obj, _parent)
            if obj.instance_collection is not None:
                ...
                _process_collection_objects(ctx, obj.instance_collection, node)
                return
        case "LIGHT" if type_supported:
            node = ctx.add_light_node(obj, _parent)
        case "CAMERA" if type_supported:
            node = ctx.add_camera_node(obj, _parent)
        case "CURVE" if type_supported:
            node = ctx.add_shape_node(obj, _parent)
        case _:
            ctx.logger.info(f"[{obj.name}] has unsupported type {obj.type!r}, exporting as Transform Group (empty)")
            node = ctx.add_transformgroup_node(obj, _parent)

    children = []
    if getattr(ctx, "_selection_set", None) is not None:
        children = [child for child in ctx._selection_set if ctx._parent_map.get(child) == obj]
    else:
    ...
    for child in sort_blender_objects_by_outliner_ordering(children):
        _add_object_to_i3d(ctx, child, node)
    logger.debug(f"[{obj.name}] no more children to process in object")


def _process_collection_objects(ctx: ExportContext, collection: bpy.types.Collection, parent: SceneGraphNode):
    ...
    for child in collection.children.values():
        _add_object_to_i3d(ctx, child, parent)
